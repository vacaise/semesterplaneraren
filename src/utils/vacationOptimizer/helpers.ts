...
// Helper function to determine if a day is a day off (weekend or holiday or company day)
export const isDayOff = (date: Date, holidays: Date[], companyDays: Date[] = []): boolean => {
  ...
  // Check if the day is a company day (klämdag)
  const isCompanyDay = companyDays.some(companyDay => isSameDay(companyDay, date));
  if (isCompanyDay) return true;
