...
import {
  ...
  OptimizedDay,
} from '@/utils/vacationOptimizer/types';
...
// Adapter function for backward compatibility with old code
export const findOptimalSchedule = (
  ...
  // Convert date holidays to string format
  const convertedHolidays = holidays.map(date => ({
    date: formatDate(date),
    name: "Holiday"
  }));

  // Map old mode strings to new strategy types
  ...
  // Call the new optimizer with adapted params
  ...
  // Convert the result to the old format
  return result.breaks.map(breakPeriod => {
    ...
    return {
      ...
      description: `${breakPeriod.totalDays} days off`,
      type: strategy,
