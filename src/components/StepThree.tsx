...
const StepThree = ({ 
  ...
  const addCompanyDay = () => {
    if (selectedCompanyDate) {
      ...
      if (!exists && !isPast(selectedCompanyDate)) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Collapsible open={isCompanyDaysOpen} onOpenChange={setIsCompanyDaysOpen} className="border rounded-lg p-4 bg-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Building2 className="h-5 w-5 text-purple-600" />
            <h4 className="font-medium text-gray-800">Företagsledighet och klämdagar</h4>
          </div>
        ...
        <CollapsibleContent className="mt-4">
          <p className="text-sm text-gray-600 mb-4">
            Lägg till företagsspecifika lediga dagar, klämdagar eller andra dagar då du inte arbetar.
          </p>
          
          <div className={`grid grid-cols-1 ${isMobile ? 'gap-4' : 'md:grid-cols-2 gap-6'} mb-4`}>
            <div className="border rounded-lg overflow-hidden bg-white p-4">
              <div className="flex items-center justify-between mb-4">
                <h5 className="font-medium text-gray-800">Välj datum</h5>
              </div>
              <div className={`${isMobile ? 'flex justify-center' : ''}`}>
                <Calendar
                  mode="single"
              ...
              >
                Lägg till företagsdag
              </Button>
            </div>

            <div className="border rounded-lg overflow-hidden bg-white p-4">
              <h5 className="font-medium text-gray-800 mb-4">Företagsdagar ({companyDays.length})</h5>
              {companyDays.length > 0 ? (
                ...
                <div className={`flex items-center justify-center ${isMobile ? 'h-[200px]' : 'h-[250px]'} bg-gray-50 rounded-md`}>
                  <p className="text-gray-500">Inga företagsdagar tillagda än</p>
                </div>
              )}
            </div>
          </div>
