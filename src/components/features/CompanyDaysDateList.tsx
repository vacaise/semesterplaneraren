...
export function DateList({ title, colorScheme }: DateListProps) {
  const { companyDaysOff, removeCompanyDay } = useCompanyDays();
  ...
  if (companyDaysOff.length === 0) {
    return (
      <div className="text-center py-3 border border-gray-200 dark:border-gray-700 rounded-md bg-white/50 dark:bg-gray-800/50">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400">No company days off selected</p>
      </div>
    );
  }
  ...
  const sortedCompanyDays = [...companyDaysOff].sort((a, b) => {
  ...
  return (
    <div className="space-y-2">
      <h4 className={cn('text-xs font-medium', headingColorClasses[colorScheme])}>
        {title} ({companyDaysOff.length})
      </h4>
      <div className="flex flex-wrap gap-2">
        {sortedCompanyDays.map((companyDay) => {
          const date = parse(companyDay.date, 'yyyy-MM-dd', new Date());
          ...
          return (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => removeCompanyDay(companyDay.date)}
                className={cn(
